...
- 사용자별 팝업 즐겨찾기 API, 카드·상세 Bookmark와 마이페이지 저장 목록
- 길안내 도착·수동 완료 기반 방문 기록과 마이페이지 날짜별 방문 목록
- 방문 기록 자격 기반 리뷰 CRUD, 검증 방문 Badge와 실시간 평균·개수 집계
- 상세 후기 탭과 마이페이지 내 리뷰 작성·수정·삭제 흐름
- 프로필·즐겨찾기·방문 기록·내 리뷰를 통합한 반응형 마이페이지
- 로그인 사용자의 즐겨찾기·방문·리뷰 상태를 일괄 조회하는 팝업 개인화 응답
- PostgreSQL 회원 스키마, 제약조건과 index 적용을 위한 참고 SQL
